...
@app.route('/application_submitted', methods=['POST'])
def applicationSubmitted(): 
  try: 
   data = request.get_json()
   user_id = data.get("user_id")
   app_id = data.get("app_id")
  
  #LoanTerms Dictionary 
   loanAmount = 10000
   interestRate = 0
   termLength = 12
   ...
   interestRate = interest_rate(user_id, app_id)
   print(interestRate)
   return jsonify({"message":  interestRate})
